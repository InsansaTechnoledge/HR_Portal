...
const navItems = [
  { icon: Home, label: 'Dashboard', path: '/home', roles: ['user', 'admin', 'superAdmin', 'accountant'] },
  {
    icon: Users,
    label: 'Management',
    roles: ['user'],
...
const SidebarItem = ({
  ...
  // const isActive = item.path === location.pathname; // Unused for parent items
    ...
    >
      <div
        className={cn(
          ...
          ({ isActive }) => (isActive ? "bg-white/20" : "group-hover:bg-sidebar-accent")
        )}
      >
        <Icon className="w-4 h-4" />
      </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {item.isAI && (
            <span className={cn(
              ...
              // To handle dynamic isActive here we might need to rely on NavLink's isActive prop or useLocation manually if we want exact styling access inside this block without render props.
              // However, NavLink children function doesn't easily expose isActive to siblings unless we wrap everything.
              // For simplicity, checking path manually for the badge style:
...
const Sidebar = () => {
  ...
  return (
    <>
      <aside
        className={cn(
          "hidden lg:flex flex-col h-screen bg-sidebar border-r",
          "transition-[width] duration-300 ease-in-out",
          isCollapsed ? "w-[72px]" : "w-[290px]"
        )}
      >
        <SidebarContent
          isCollapsed={isCollapsed}
      ...
      <aside className={cn(
        ...
        <SidebarContent
          isMobile
          isCollapsed={isCollapsed} // Mostly N/A for mobile but passed to keep props consistent if needed or ignored
          navItems={filteredNavItems}
          user={user}
          openMenus={openMenus}
          setOpenMenus={setOpenMenus}
